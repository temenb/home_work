...
class ActionRegistration extends ActionAbstract
{
    ...
    public function run()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($stmt = mysqli_prepare($link, "select * from user where email= ?")) {
            ...
            if ($validaton) {
                ...
                $stmt = mysqli_prepare(
                    $link,
                    "INSERT INTO user (id, root_name, email, password, salt) VALUES (NULL,'user',?,?,?)"
                );
                if ($stmt) {
                    ...
                    if ($success) {
                        $this->messages['success'][] = 'Congrads, you are in! Please login.';
